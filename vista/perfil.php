...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['guardar']) && isset($_POST['etiquetas'])) {
  ...
  header("Location: " . $_SERVER['PHP_SELF'] ."?msj=1"); // Recarga la pagina despues de guardar etiquetas
  exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['quitar'])) {
  ...
  header("Location: " . $_SERVER['PHP_SELF'] ."?msj=1"); // Recarga la pagina despues de quitar etiquetas
  exit();
}
